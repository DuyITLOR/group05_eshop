# TC-FR16-27

## Requirement ID
FR16

## Module / Test type / Technique
Import Products / Functional / Boundary Value Analysis

## Test design source
test-design/BVA-FR16.md

## Coverage
| Coverage Type | Covered IDs |
|---|---|
| Boundary Value Analysis | FR16_BVA_10 |

## Detail
| ID | Partition Tested | File CSV (Input) | Expected Output |
|---|---|---|---|
| FR16_BVA_10 | Giá trị `price` < 0 (Số âm, biên ngoài) | File `.csv` (EXT1), Header đúng (HD1), dòng dữ liệu:<br>`Sản phẩm A,-1,,,1` | - Không thêm sản phẩm nào vào DB (DB2)<br>- Hiển thị thông báo lỗi: "Hàng 2: Giá tiền không hợp lệ (phải là số dương)" (REP3) |

## Preconditions
1. Người dùng đăng nhập hệ thống với tài khoản Admin (`role = 'admin'`).
2. Người dùng đang ở màn hình Import sản phẩm từ file CSV.

## Test data
| Field | Value |
|---|---|
| File Content | name,price,description,imageUrl,category_id<br>Sản phẩm A,-1,,,1 |

## Test steps
1. Tải lên file CSV chứa sản phẩm có giá bằng -1.
2. Nhấp nút "Import".

## Expected results
1. Hệ thống từ chối import và không thêm sản phẩm nào vào cơ sở dữ liệu.
2. Hiển thị thông báo lỗi chi tiết: "Hàng 2: Giá tiền không hợp lệ (phải là số dương)".

## Actual results
1. Thất bại: Hệ thống cho phép import sản phẩm có giá âm (-1) vào DB.
2. Thất bại: Không hiển thị thông báo lỗi giá tiền không hợp lệ.

## Status
Failed
