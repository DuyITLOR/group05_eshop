...
const csvPath = path.resolve(
  __dirname,
  "../test-data/accounts_endurance.csv",
);
const sqlite3 = require(sqlite3Path).verbose();
const db = new sqlite3.Database(databasePath);

const USER_COUNT = 500;
const PASSWORD = "Endurance1234!";
const products = [
...
for (let index = 1; index <= USER_COUNT; index += 1) {
  ...
  csvRows.push(
    [
      `endurance${suffix}@eshop.test`,
      PASSWORD,
      product.id,
      product.name,
      product.price,
      1,
      `Endurance Test Address ${suffix} HCMC`,
    ].join(","),
  );
}
...
db.serialize(() => {
  ...
  for (let index = 1; index <= USER_COUNT; index += 1) {
    const suffix = String(index).padStart(3, "0");
    const email = `endurance${suffix}@eshop.test`;
    update.run(PASSWORD, email);
    insert.run(`Endurance User ${suffix}`, email, PASSWORD, email);
  }

  update.finalize();
  insert.finalize(() => {
    db.get(
      "SELECT COUNT(DISTINCT email) AS count FROM users WHERE email LIKE 'endurance%@eshop.test'",
      (error, row) => {
        if (error) {
          console.error(error.message);
          process.exitCode = 1;
        } else {
          console.log(`Da chuan bi ${row.count} tai khoan Endurance trong ${databasePath}`);
