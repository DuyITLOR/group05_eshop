# TC-FR16-02

## Requirement ID
FR16

## Module / Test type / Technique
Import Products / Functional / Equivalence Partitioning

## Test design source
test-design/EP-FR16.md

## Coverage
| Coverage Type | Covered IDs |
|---|---|
| Equivalence Partitioning | FR16_EP_IV01 |

## Detail
| ID | Test Objective | File CSV (Input) | Database State (Output) | UI Report (Output) |
|---|---|---|---|---|
| FR16_EP_IV01 | Tải lên file sai đuôi mở rộng (định dạng `.xlsx`) | File `products.xlsx` (EXT2), chứa dữ liệu hợp lệ | Không có sản phẩm nào được thêm vào DB (DB2) | Báo lỗi định dạng file không hợp lệ (REP2) |

## Preconditions
1. Người dùng đăng nhập hệ thống với tài khoản Admin (`role = 'admin'`).
2. Người dùng đang ở màn hình Import sản phẩm từ file CSV.

## Test data
| Field | Value |
|---|---|
| File Name | products.xlsx |
| File Content | File bảng tính Excel chứa dữ liệu sản phẩm hợp lệ |

## Test steps
1. Chọn file `products.xlsx` để tải lên hệ thống.
2. Nhấp nút "Import".

## Expected results
1. Hệ thống từ chối import và hiển thị thông báo lỗi định dạng file không hợp lệ (yêu cầu file CSV).
2. Không có sản phẩm nào được thêm vào cơ sở dữ liệu.

## Actual results
1. Thất bại: Hệ thống không kiểm tra đuôi file, vẫn gửi file '.xlsx' lên API. FileReader đọc file binary thành chuỗi text lỗi và gửi lên API.
2. Thành công: Không có sản phẩm nào được lưu vào cơ sở dữ liệu (do lỗi định dạng trống tên).
3. Thất bại: Giao diện hiển thị thông báo import hoàn tất với các lỗi dòng thay vì từ chối định dạng file.

## Status
Failed
