# TC-REGISTER-003: Đăng ký với email sai định dạng

## Requirement ID
FR-01

## Module / Test type / Technique
Register / Functional / Equivalence Partitioning (vùng không hợp lệ EP4 — email sai định dạng, thiếu `@`)
...
| --- | --- |
| name | `Nguyen Van A` |
| email | `abc` *(thiếu `@`, không đúng `local@domain.tld`)* |
...
## Actual result
API tạo user với email `abc` (HTTP 200). Backend không validate định dạng email.
